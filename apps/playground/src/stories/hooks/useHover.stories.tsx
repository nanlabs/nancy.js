...
export const Example = () => {
  ...
  const getEventHandler = useCallback(
    (eventName: string) => {
      return () =>
        setEvents((events) => [...events, `"${eventName}" callback fired`]);
    },
    [setEvents]
  );
  ...
  return (
    <div style={{ display: "flex", gap: 10 }}>
      <div style={hoverableStyles} ref={divRef}>
        <h2>{isHovered ? "Hovered" : "Hover me"}</h2>

        <span>
          <button onClick={() => setIsMemoEnabled((val) => !val)}>
            {isMemoEnabled
              ? "Callbacks memoized"
              : "Callbacks change each render"}
          </button>
        </span>
        ...
        <span>
          <button onClick={() => setIsRefTarget((val) => !val)}>
            {isRefTarget
              ? "Target provided as ref"
              : "Target provided as function"}
          </button>
        </span>
      </div>
      <div>
